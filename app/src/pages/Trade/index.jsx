  ...
  const editTabCell4Number = (cell, record) => {
    ...
    if(editingRow.outId === record.outId){
      ...
      return (
        ...
        }>{cell.valueType==='money'?'¥':''}{formatCurrency(domValue)}</a>
      );
    }
  };
        ...
        <FooterToolbar
          extra={
            <div>
              ...
              <span>
                {' 总投入金额：'}
                {formatCurrency(totalMoney = selectedRowsState.reduce((pre, item) => pre + parseFloat(item['stock&totalFee']), 0))}{'元('}
                {formatCurrency(totalMoney/10000.0)}{'万)'}
              </span>
            </div>
          }
        >
